 ...
 */
public class AddPermissionCommand extends MPlayerCommand {


    public AddPermissionCommand(String name, String description, String... aliases) {
    ...
    public boolean execute(MooPermissions instance, String playerName, String[] args, CommandSender commandSender) {
        ...
        if (mPlayer.addPermissionToPlayer(args[0], give, time)) {
            StringManager.sendMessage(commandSender, "&bSuccessfully added &3%s &bto &3%s", args[0], mPlayer.getName());
        } else {
            StringManager.sendMessage(commandSender, "&bProblem when adding &3%s &bto &3%s", args[0], mPlayer.getName());
