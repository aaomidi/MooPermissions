 ...
 */
public class DelAllPermissionsCommand extends MPlayerCommand {
    public DelAllPermissionsCommand(String name, String description, String... aliases) {
    ...
    public boolean execute(MooPermissions instance, String playerName, String[] args, CommandSender commandSender) {
        ...
        boolean result = mPlayer.removeAllPermissions();

        if (result) {
            StringManager.sendMessage(commandSender, "&bSuccessfully removed all permissions from &3%s", mPlayer.getName());
        } else {
            StringManager.sendMessage(commandSender, "&bProblem when removing all permissions from &3%s", mPlayer.getName());
        }
